 ...
 * @date: 27/03/2018
 */
public class SigninForThird extends Signin {
    public ResultMsg loginForQQ(String openId) {
        ...
        //3.注册(在原有的系统里面创建一个用户)
        ResultMsg registMsg = super.regist(openId, pwd);
        //4.调用原来的登录方法
        ResultMsg loginMsg = super.login(openId, pwd);
        return loginMsg;
    }
    ...
    public ResultMsg loginForRegist(String username,String password) {
        ResultMsg registMsg = super.regist(username, password);
        //4.调用原来的登录方法
        ResultMsg loginMsg = super.login(username, password);
        return loginMsg;
    }
}
